...
const Field = forwardRef<FieldInstance, FieldProps>((props, ref) => {
  ...
  const renderLeftIcon = () => {
    ...
    return (
      <Pressable onPress={props.onClickLeftIcon}>{props.leftIcon}</Pressable>
    );
  };
  ...
  const renderInput = useMemoizedFn(() => {
    ...
    return (
      <Input ref={ref} type={props.type} align={props.align} {...commonProps} />
    );
  });
  ...
  const renderIntro = () => {
    if (props.intro) {
      return (
        <Text style={[styles.intro, { textAlign: align }]}>{props.intro}</Text>
      );
    }
    return null;
  };
